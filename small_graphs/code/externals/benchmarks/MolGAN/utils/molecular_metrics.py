...
parent_dir = str(str(Path(os.path.abspath(__file__)).parents[1]))
...
NP_model = pickle.load(gzip.open(parent_dir + "/data/NP_score.pkl.gz"))
SA_model = {
    i[j]: float(i[0])
    for i in pickle.load(gzip.open(parent_dir + "/data/SA_score.pkl.gz"))
...
class MolecularMetrics(object):
    ...
    def _compute_SAS(mol):
        ...
        # This form generates better results_v1 when 2 or more macrocycles are present
        if nMacrocycles > 0:
