...
def loss_func(score_list, grad_log_q_noise_list, sigma_list):
    loss = 0.0
    loss_items = []
    for score, grad_log_q_noise, sigma in zip(
        score_list, grad_log_q_noise_list, sigma_list
    ):
        cur_loss = (
            0.5
            * sigma ** 2
            * ((score - grad_log_q_noise) ** 2).sum(dim=[-1, -2]).mean()
        )
...
def fit(
    ...
    for epoch in range(max_epoch):
        ...
        for train_adj_b, train_x_b in train_dl:
            ...
            ) = gen_list_of_data(train_x_b, train_adj_b, train_node_flag_b, sigma_list)
            optimizer.zero_grad()
            # print('config.dev: ', config.dev)
            score = model(
                x=train_x_b, adjs=train_noise_adj_b, node_flags=train_node_flag_b
            )

            loss, loss_items = loss_func(
                score.chunk(len(sigma_list), dim=0), grad_log_q_noise_list, sigma_list
            )
        ...
        for test_adj_b, test_x_b in test_dl:
            ...
            (
                ...
                test_node_flag_b,
                grad_log_q_noise_list,
            ) = gen_list_of_data(test_x_b, test_adj_b, test_node_flag_b, sigma_list)
            with torch.no_grad():
                score = model(
                    x=test_x_b, adjs=test_noise_adj_b, node_flags=test_node_flag_b
                )
            loss, loss_items = loss_func(
                score.chunk(len(sigma_list), dim=0), grad_log_q_noise_list, sigma_list
            )
        ...
        logging.info(
            f"epoch: {epoch:03d}| time: {time.time() - t_start:.1f}s| "
        ...
        if epoch % save_interval == save_interval - 1:
            ...
            torch.save(
                to_save,
                os.path.join(
                    config.model_save_dir, f"{config.dataset.name}_{sigma_list}.pth"
                ),
            )
        ...
        if epoch % sample_interval == sample_interval - 1:
            ...
            step_size = (
                ...
                * torch.tensor(sigma_list)
                .to(test_x_b)
                .repeat_interleave(test_adj_b.size(0), dim=0)[..., None, None]
                ** 2
            )
            test_node_flag_b = test_adj_b.sum(-1).gt(1e-5).to(dtype=torch.float32)
            (
            ...
            with torch.no_grad():
                sample_b, _ = mcmc_sampler.sample(
                    config.sample.batch_size,
                    lambda x, y: model(test_x_b, x, y),
                    max_node_num=max_node_number,
                    step_num=None,
                    init_adjs=init_adjs,
                    init_flags=test_node_flag_b,
                    is_final=True,
                    step_size=step_size,
                )
                sample_b_list = sample_b.chunk(len(sigma_list), dim=0)
                init_adjs_list = init_adjs.chunk(len(sigma_list), dim=0)
                for sigma, sample_b, init_adjs in zip(
                    sigma_list, sample_b_list, init_adjs_list
                ):
                    sample_from_sigma = sigma + sample_from_sigma_delta
                    eval_sample_batch(
                        sample_b,
                        mcmc_sampler.end_sample(test_adj_b, to_int=True)[0],
                        init_adjs,
                        config.save_dir,
                        title=f"epoch_{epoch}_{sample_from_sigma}.pdf",
                    )

                    if init_adjs is not None:
                        plot_graphs_adj(
                            mcmc_sampler.end_sample(init_adjs, to_int=True)[0],
...
if __name__ == "__main__":
    ...
    args = parse_arguments("train_ComSmall20.yaml")
