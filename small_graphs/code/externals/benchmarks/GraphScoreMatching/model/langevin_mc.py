...
class LangevinMCSampler(object):
    ...
    def gen_init_sample(self, batch_size, max_node_num, node_flags=None):
        adjs_c = (
            torch.randn((batch_size, max_node_num, max_node_num), dtype=torch.float32)
            .triu(diagonal=1)
            .abs()
            .to(self.dev)
        )
    ...
    def adj_to_int(adjs_c, to_int=False):
        if to_int:
            adjs_d = torch.where(
                adjs_c < 0.5, torch.zeros_like(adjs_c), torch.ones_like(adjs_c)
            )
        else:
    ...
    def sample(
        ...
        for step in range(step_num):
            take_log = step % 1 == 0
            adjs, node_flags = self._step_sample(
                step, score_func, adjs, node_flags, log=take_log
            )
            check_adjs_symmetry(adjs)
        adjs.detach_()
        node_flags.detach_()
    ...
    def _step_sample(self, step, score_func, adjs, node_flags, log=True):
        ...
        if log:
            logging.debug(
                f"LG MC: step {step:5d}\t|" + "score: {:+.2e}\t|new_score_d: {:+.2e}\t"
                "|adj_mean: {:.2e}\t|adj_std: {:.2e}\t|delta_abs_mean: {:+.2e}\t|".format(
                    score.norm(dim=[-1, -2]).mean().item(),
                    score_func(self.adj_to_int(adjs_c, to_int=False)[0], node_flags)
                    .norm(dim=[-1, -2])
                    .mean()
                    .item(),
