...
def process_config(config, comment=""):
    ...
    if "save_dir" not in config:
        config.save_dir = os.path.join(
            config.exp_dir, config.exp_name, config.folder_name
        )
...
def graphs_to_tensor(config, graph_list):
    ...
    for g in graph_list:
        ...
        padded_feature_np = np.concatenate(
            [
                feature_np,
                np.zeros(
                    [config.dataset.max_node_num - feature_np.shape[0], feature_len]
                ),
            ],
            axis=0,
        )
...
def load_data(config, get_graph_list=False):
    graph_list, node_feature_set = load_dataset(
        data_dir="data", file_name=config.dataset.name
    )
    ...
    train_graph_list, test_graph_list = (
        graph_list[test_size:],
        graph_list[:test_size],
    )
    if get_graph_list:
        return train_graph_list, test_graph_list
    return (
        graphs_to_dataloader(config, train_graph_list),
