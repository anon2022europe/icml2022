...
from ggg.data.backport import DatasetHpars as DatasetHparsNew
from ggg_utils.utils.logging import register_exp as register_exp_new, register_trainer as register_trainer_new, \
    register_plm as register_plm_new
...
class GGG_Hpar(AbstractConf):
    ...
    def children(cls) -> Dict:
        return dict(
            ...
            dataset_hpars=DatasetHparsNew,  # switching to new one
        )
    ...
    dataset_hpars = attr.ib(default=DatasetHparsNew())
    ...
    def __attrs_post_init__(self):
        ...
        elif self.root_hpars.name in {"noneq"}:
            assert self.root_hpars.context_dim == self.trunk_hpars.feat_dim
        else:
    ...
    def report_base(cls,
                    ...
                    trunk="attention",
                    root="geometric",
                    dts_limit_train=None,
                    dataset="nx_star",
                    nx_kwargs: Optional[Dict] = None,
                    num_labels=None,
                    node_feature=0, n_heads=5,
                    edge_readout="attention_weights",
                    on=("real", "fake", "real-perturbed", "fake-perturbed"),
                    modes=("LP", "LP", "LP", "LP"), eigenfeat4_=False,
                    conv_channels=[32, 64, 64, 128], noise_dist_trunk="normal",
                    structured_features=True, use_laplacian=True,
                    exp_name="GG", extra_features=0, penalty_lambda=10,
        ...
        hpars = GGG_Hpar(exp_name=exp_name, gen_every=gen_every,
                         ...
                         penalty_hpars=GradPenHpars(on=on, penalty_agg=penalty_agg, input_agg=input_agg,
                                                    modes=modes, penalty_lambda=penalty_lambda),
                         )
...
class GGG(LightningModule):
    def __init__(self, hparams):
        ...
        weights=self.train_set.node_dist_weights()
        num_node_weights = ensure_tensor(weights)
    ...
    def forward(self, batch_size=None, Z=None, X=None, A=None, N=None, device=None):
        ...
        if self.hpars.edge_readout_hpars.name=="noneq":
            Xf=Xf.repeat(1,Af.shape[-1],1)
        if Xf is not None:
            assert (
                    Xf.shape[-1]
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        register_exp_new(self.logger.experiment)
        register_trainer_new(self.trainer)
        register_plm_new(self)
    ...
    def validation_set(self):
        return self.hpars.dataset_hpars.make_val()
