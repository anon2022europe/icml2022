...
class AttentionEdgeReadout(nn.Module):
    ...
    def forward(self, X, A=None, N=None) -> torch.Tensor:
        Z = X
        Zi = Z
        att, _, scores = self.attn.forward(q=Zi, k=Zi, v=Zi, return_attention_and_scores=True)
        if self.readout == "sigmoid":
            ...
            temp_attn, _, temp_scores = self.score_attn.forward(
            ...
            _, _, scores = self.score_attn.forward(
                att, att, att, return_attention_and_scores=True
            )
