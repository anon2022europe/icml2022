...
def get_dist(dist):
    if isinstance(dist, pt.distributions.Distribution):
        ...
        return pt.distributions.Bernoulli(probs=0.5)
    else:
...
class RandomRoot(GeneratorRoot):
    def forward(self, batch_size, device, Z=None, N=None, joined_embed=None):
        if N is None:
...
class NonEqRoot(GeneratorRoot):
    def __init__(
        ...
        super().__init__(n_node_dist, node_embedding_dim, noise_dist)
        self.context_dim = get_dist(joined_noise_dist)
        self.context_dim = context_dim

    def forward(self, batch_size, device, Z=None, N=None, joined_embed=None):
        # fake N
        noise=self.noise_dist.sample((batch_size,1,self.context_dim)).to(device)
        return noise,None, N, None
