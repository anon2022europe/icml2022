...
try:
    ...
    from pytorch_lightning.callbacks.lr_monitor import LearningRateMonitor as LearningRateLogger
...
def run(
    ...
    if benchmark is not None:
        warn(f"Doctoring with the config to add the {benchmark} benchmark")
        from ggg_data.icml2022_benchmarks import BENCHMARKS
        benchmark_dict=BENCHMARKS[benchmark]().to_dict()
        hparams["dataset_hpars"]=benchmark_dict
