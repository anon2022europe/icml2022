import os
from ggg_utils.utils.sbm_fit import nx_fitted_sbm_nll_per_edge
import csv
import pickle
import warnings
...
def main_run_MMD(
    ...
    if dataset_name is not None and "communitysmall" in dataset_name.lower():
        print("SBM LL ")
        dataset_sbm_ll=pt.tensor([nx_fitted_sbm_nll_per_edge(g,seed=0,comms=None,use_entropy=True) for g in tqdm(dataset_graphs_cut,leave=False,desc="DS SBM LL")])
        model_sbm_ll = pt.tensor([nx_fitted_sbm_nll_per_edge(g, seed=0, comms=None, use_entropy=True) for g in tqdm(model_graphs_cut,leave=False,desc="MODEL SBM LL")])
        sbm_lls=[x.mean().item() if isinstance(x,pt.Tensor) else x for x in (dataset_sbm_ll,model_sbm_ll)]
        print("SBM LL done ")
    else:
        sbm_lls=None
    ...
    (
        ...
        x.item() if pt.is_tensor(x) else x
        for x in [
            degree_metric,
            clustering_metric,
            cycle_metric,
    ...
    if save:
        ...
        if (
            ...
            algebraic_connectivity_metric = algebraic_connectivity_metric.item() if hasattr(algebraic_connectivity_metric,"item") else float(algebraic_connectivity_metric)
        write_to_csv(
    ...
    ret=[
        ...
        degree_assortativity_metric,
        algebraic_connectivity_metric,
    ]+(list(sbm_lls) if sbm_lls is not None else [])
    return ret
        ...
        with open(os.path.join(csv_dir, "stats.csv"), "w", newline="") as _file:
            writer = csv.DictWriter(
                ...
                fieldnames=[
                    ...
                    "mmd_eccentricity",
                ]+(["dataset_sbm_ll","model_sbm_ll"] if sbm_lls is not None else []),
            )
            writer.writeheader()
    data = [
        model,
...
def generate_graphs(
    ...
    while len(generated_graphs) < numb_graphs:
        ...
        sout = [
            x
            for x in getattr(model,"generator",model).sample(
    ...
    if dataset is not None:
        dataset_graphs = get_dataset_graphs(dataset)
...
def get_dataset_graphs(dataset):
    key = dataset
    dataset_graphs=[]
