...
class RingOfCliques(BaseNxSet):
    def __init__(self, num_cliques=2,clique_sizes=2,repeat=1):
        ...
        self.graphs=[nx.ring_of_cliques(n,c) for n,c in zip(num_cliques,clique_sizes)]*repeat
...
class LolliPop(BaseNxSet):
    def __init__(self,N_path=2, N_cluster=3,repeat=1):
        ...
        self.graphs=[nx.lollipop_graph(k,p) for p,k in zip(N_path,N_cluster)]*repeat
    def __getitem__(self,idx):
        return None,self.graphs[idx]
    def __len__(self):
        return len(self.graphs)
...
NX_CLASSES=dict(nx_star=StarGraph,nx_circ_ladder=CircularLadder,nx_lollipop=LolliPop,nx_roc=RingOfCliques,nx_combo=RocAndCircL)

class NXGraphWrapper(Dataset):
    def __init__(
            self, clsname, *dataset_args, **dataset_kwargs
    ):
