...
def sample_main(ori_config, args):
    ...
    def gen_init_data(batch_size):
        ...
        node_flags = (
            base_adjs.sum(-1).gt(1e-5).to(dtype=torch.float32)
        )  # mark isolate nodes as non-exist
        print(
            batch_size,
            node_flags.size(0),
            base_adjs.size(0),
        ...
        if isinstance(
            config.init_sigma, float
        ):  # when generate from perturbed training data
            base_adjs, _ = add_gaussian_noise(
                base_adjs, node_flags=node_flags, sigma=config.init_sigma
            )
        else:  # when generate from noise
            base_adjs = mcmc_sampler.gen_init_sample(
                batch_size, max_node_number, node_flags=node_flags
            )[0]
    ...
    def run_sample(
        ...
        warm_up_count = 0
        gen_graph_list = []
        init_adjs, init_x, node_flags = gen_init_data(
            batch_size=test_batch_size * len(sigma_list)
        )
        sample_x = init_x
        ...
        while len(gen_graph_list) < eval_len:
            step_size = (
                ...
                * torch.tensor(sigma_list)
                .to(init_adjs)
                .repeat_interleave(test_batch_size, dim=0)[..., None, None]
                ** 2
            )
            # print(f'ss: {config.mcmc.grad_step_size}, {sigma_list}, {step_size}')
            with torch.no_grad():
                sample_adjs, _ = mcmc_sampler.sample(
                    config.sample.batch_size,
                    lambda x, y: model(sample_x, x, y),
                    max_node_num=max_node_number,
                    step_num=None,
                    init_adjs=init_adjs,
            ...
            if warm_up_count < len(sigma_list):
                if validation:
                    for ii in range(len(sigma_list)):
                        ...
                        #                                      separator='\t', prefix='\t'))
                        pic_title = (
                            f'{file.split("/")[-1]}_{step_size_ratio}_{eps}_'
                            f"{warm_up_count}_{sigma_list[ii]}_sample.pdf"
                        )
                        eval_sample_batch(
                            rounded_adjs,
                            valid_adj_b_ori,
                            init_adjs.chunk(len(sigma_list), dim=0)[ii],
                            config.save_dir,
                            title=pic_title,
                        )
                        result_dict = eval_torch_batch(
                            valid_adj_b_ori, rounded_adjs, methods=methods
                        )
                        logging.info(
                            f"MMD {file} {warm_up_count} iter at {sigma_list[ii]}: {result_dict}"
                        )
                        logging.info(f"save fig at {config.save_dir} {pic_title}")
                warm_up_count += 1
            else:
                gen_graph_list.extend(
                    adjs_to_graphs(
                        mcmc_sampler.end_sample(sample_adjs_list[0])[0].cpu().numpy()
                    )
                )
            new_init_adjs, new_init_x, new_node_flags = gen_init_data(
                batch_size=test_batch_size
            )
            init_adjs = torch.cat(list(sample_adjs_list[1:]) + [new_init_adjs], dim=0)
            sample_x = torch.cat(
                [sample_x[sample_adjs_list[0].size(0) :], new_init_x], dim=0
            )
            sample_node_flags = torch.cat(
                [sample_node_flags[sample_adjs_list[0].size(0) :], new_node_flags],
                dim=0,
            )
        pic_title = f'{file.split("/")[-1]}_{step_size_ratio}_{eps}_{sigma_list}_final_sample.pdf'
        plot_graphs_list(
            graphs=gen_graph_list, title=pic_title, save_dir=config.save_dir
        )
    ...
    for step_size_ratio in step_size_list:
        for eps in eps_list:
            valid_result_dict, _ = run_sample(
                target_graph_list=train_graph_list[: config.test.batch_size],
            ...
            if np.sum(list(valid_result_dict.values())) < np.sum(
    ...
    with open(
        os.path.join(
            sample_dir,
            file + f"_{best_config['step_size_ratio']}"
            f"_{best_config['eps']}_sample.pkl",
        ),
        "wb",
    ) as f:
