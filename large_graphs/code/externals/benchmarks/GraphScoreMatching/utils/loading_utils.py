...
def gen_random_feature(batch_size, max_node_num, in_feature, dev, one_hot=False):
    if in_feature == 0:
        ...
        if one_hot:
            ...
            train_node_feature_b = (
                torch.eye(max_node_num).unsqueeze(0).repeat([batch_size, 1, 1])
            )
        else:
            train_node_feature_b = (
                torch.randn(batch_size, max_node_num, in_feature) * 0.3
            )
...
def get_norm_fit_stats(data, string=True):
    ...
    if string:
        return (
            "\t".join([f"{v:.2f}" for v in [loc.item(), scale.item(), kss]])
            + f"\t{p:.2e}"
        )
...
def get_mc_sampler(config):
    if config.mcmc.name == "langevin":
        mc_sampler = LangevinMCSampler(
            eps=config.mcmc.eps
            if isinstance(config.mcmc.eps, float)
            else config.mcmc.eps[0],
...
def get_score_model(config, dev=None, **kwargs):
    ...
    if config.model.name == "gnn":
        ...
        score_model = ScoreNetwork(
            ...
            feature_num=sum(feature_nums),
            stack_num=config.model.stack_num if "stack_num" in config.model else 1,
        ).to(dev)
    elif config.model.name == "mlp":
        score_model = MLPScoreNetwork(
            nef=64, max_node_number=config.dataset.max_node_num, dev=dev
        ).to(dev)
    elif config.model.name == "cov":
        score_model = ConvScore(
            nef=64, max_node_number=config.dataset.max_node_num, dev=dev
        ).to(dev)
    elif config.model.name == "unet":
        score_model = UNetResScore(
            nef=64, max_node_number=config.dataset.max_node_num, dev=dev
        ).to(dev)
        ...
        score_model = EdgeDensePredictionGraphScoreNetwork(
            ...
            gnn_module_func=gnn_model_func,
            dev=dev,
            num_classes=len(config.train.sigmas),
        ).to(dev)
    else:
...
def eval_sample_batch(sample_b, test_adj_b, init_adjs, save_dir, title=""):
    ...
    round_init_adjs = torch.where(
        init_adjs < 0.5, torch.zeros_like(init_adjs), torch.ones_like(init_adjs)
    )
    round_init_delta = round_init_adjs - test_adj_b
    logging.info(
        f"sample delta_norm_mean: {delta.norm(dim=[1, 2]).mean().item():.3e} "
...
def prepare_test_model(config):
    ...
    for file in config.model_files:
        ckp = torch.load(
            os.path.join(config.model_save_dir, file), map_location=config.dev
        )
    ...
    )  # large sigma_list -> small sigma_list
    return models
