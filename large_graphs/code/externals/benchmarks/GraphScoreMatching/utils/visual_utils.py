...
options = {"node_size": 2, "line_color": "black", "linewidths": 1, "width": 0.5}
            ...
            for (u, v, d) in g.edges(data=True):
                ...
                for group_i, weight_range in enumerate(weight_ranges):
                    if weight_range[0] < weight <= weight_range[1]:
                        ...
                        break
            for e_group, w_group, cmap, alpha, width in zip(
                e_groups, w_groups, cmap_list, alphas, widths
            ):
...
def plot_multi_channel_numpy_adjs_1b1(
    adjs, title="multi_channel_viz", save_dir=None, fig_dir="fig"
):
    ...
    for layer_i, adj in enumerate(adjs):
        ...
        for channel_i in range(adj.shape[0]):
            ...
            colors5 = plt.cm.twilight_shifted(np.linspace(0.0 + k, 1 - k, 128))
            ...
            save_fig(
                save_dir=save_dir,
                title=file_name + f"_l{layer_i}_c{channel_i}_m" + suffix,
                dpi=300,
                fig_dir=fig_dir,
            )
            ...
            def draw_a_graph(g, g_suffix="g"):
                ...
                save_fig(
                    save_dir=save_dir,
                    title=file_name + f"_l{layer_i}_c{channel_i}_{g_suffix}" + suffix,
                    dpi=300,
                    fig_dir=fig_dir,
                )
...
def save_fig(save_dir=None, title="fig", dpi=300, fig_dir="fig"):
    ...
    if save_dir is None:
        ...
        if not os.path.exists(fig_dir):
            os.makedirs(fig_dir)
        plt.savefig(
            os.path.join(fig_dir, title), bbox_inches="tight", dpi=dpi, transparent=True
        )
        plt.close()
    return
...
def plot_graphs_list(
    graphs, energy=None, node_energy_list=None, title="title", max_num=16, save_dir=None
):
    batch_size = len(graphs)
    ...
    for i in range(max_num):
        ...
        if node_energy_list is not None:
            ...
            nx.draw(
                G, with_labels=False, node_color=node_energy, cmap=cm.jet, **options
            )
        else:
...
def plot_graphs_adj(
    adjs, energy=None, node_num=None, title="title", max_num=16, save_dir=None
):
