...
class UNetResScore(nn.Module):
    def __init__(self, nef, max_node_number, dev):
        ...
        ngf = nef
        unet_block = UnetSkipConnectionBlockWithResNet(
        ...
        self.model = UnetSkipConnectionBlockWithResNet(
            output_nc, ngf * 2, input_nc=input_nc, submodule=unet_block, outermost=True
        )  # add the outermost layer
        self.mask = torch.ones([max_node_number, max_node_number]) - torch.eye(
            max_node_number
        )
    ...
    def forward(self, x, adjs, node_flags):
        ...
        out = F.adaptive_max_pool2d(
            input=out, output_size=[self.max_node_num, self.max_node_num]
        )
...
class ConvScore(nn.Module):
    def __init__(self, nef, max_node_number, dev):
        ...
        self.conv_net = nn.Sequential(
            ...
            nn.ELU(),
            nn.ConvTranspose2d(
                max(1, nef // 2), max(1, nef // 4), 3, stride=1, padding=1
            ),
            # nn.Softplus()
            nn.ELU(),
            nn.ConvTranspose2d(
                max(1, nef // 4), max(1, nef // 8), 1, stride=1, padding=0
            ),
            # nn.Softplus()
            nn.ELU(),
...
class ScoreNetwork(nn.Module):
    def __init__(self, gnn_module_func, feature_num, stack_num=2):
        ...
        for _ in range(stack_num):
            self.score_net_list.append(
                OneLayerScoreNetwork(gnn_module_func(), feature_num)
            )
