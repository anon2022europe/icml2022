...
class GIN(GraphNeuralNetwork):
        ...
        **kwargs
        ...
        def linear_with_leaky_relu(ii):
            return nn.Sequential(
                nn.Linear(feature_nums[ii], hidden_num),
                nn.LeakyReLU(),
        ...
        for i in range(layer_n):
            if isinstance(attention_heads, list):
                ...
                attentions = [
                    ...
                    for _ in range(attention_heads[i + 1])
                ]
                mlp = MLP(
                    num_layers=2,
    ...
    def _aggregate(self, x, adjs, node_flags, layer_k):
        ...
        if self.use_norm_layers:
            x = (
                self.norm_layers[layer_k](x.view(-1, feature_num))
                .contiguous()
                .view(batch_size, -1, feature_num)
            )
        if self.attentions is None:
            if len(adjs.shape) == 4:
                ...
                h = (
                    h.permute(0, 2, 1, 3)
                    .contiguous()
                    .view(adjs.size(0), adjs.size(-1), -1)
                )  # B x N x CF
            else:
                h = torch.bmm(adjs, x)
    ...
    def _readout(self, x, adjs, node_flags):
        ...
        for layer, h in enumerate(self.hidden):
            ret = ret + F.dropout(
                self.linear_prediction[layer](h), self.dropout_p, training=self.training
            )
