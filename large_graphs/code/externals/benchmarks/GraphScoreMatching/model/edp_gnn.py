...
class EdgeDensePredictionGNNLayer(nn.Module):
    ...
    def forward(self, x, adjs, node_flags):
        ...
        new_adjs = mlp_out.view(
            mlp_in_shape[0], mlp_in_shape[1], mlp_in_shape[2], -1
        ).permute(0, 3, 1, 2)
...
class EdgeDensePredictionGraphScoreNetwork(nn.Module):
    def __init__(
        ...
        for i in range(gnn_layer_num):
            gnn_feature_list = [
                feature_num_list[i] + channel_num_list[i]
            ] + gnn_hidden_num_list
            gnn = gnn_module_func(
    ...
    def forward(self, x, adjs, node_flags, viz=False, save_dir=None, title=""):
        ...
        adjs = torch.cat([ori_adjs, 1.0 - ori_adjs], dim=1)  # B x 2 x N x N
