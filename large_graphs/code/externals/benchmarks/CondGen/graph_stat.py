...
def statistics_claw_count(A_in):
    ...
    degrees = A_in.sum(axis=0).flatten()
    return float(np.sum(np.array([1 / 6.0 * x * (x - 1) * (x - 2) for x in degrees])))
...
def statistics_gini(A_in):
    ...
    G = (2 * np.sum(np.array([i * degrees_sorted[i] for i in range(len(degrees))]))) / (
        n * np.sum(degrees)
    ) - (n + 1) / n
    return float(G)
...
def statistics_edge_distribution_entropy(A_in):
    ...
    H_er = (
        ...
        / np.log(n)
        * np.sum(
            -degrees / (2 * float(m)) * np.log((degrees + 0.0001) / (2 * float(m)))
        )
    )
    return H_er
...
def compute_graph_statistics(A_in, Z_obs=None):
    ...
    # Clustering coefficient
    statistics["clustering_coefficient"] = (
        3 * statistics["triangle_count"] / (statistics["claw_count"] + 1)
    )
