...
def load_our_data(dataset):
    if dataset in {"MolGAN_5k", "CommunitySmall_20", "anu_graphs_chordal9","CommunitySmall_100"}:
        if dataset == "MolGAN_5k":
            dataset = "MolGAN_5k"
            filename = "QM9_5k.sparsedataset"
        elif "Community" in dataset:
    ...
    for i in tqdm(range(len(ds)), desc="Unpacking"):
        ...
        if torch.is_tensor(A):
            A = A.cpu().numpy()
        adj_mats.append(A)
    train_adj_mats = adj_mats[: int(len(adj_mats) * 1.0)]
...
def train(
    ...
    for epoch in tqdm(range(max_epochs), desc="Epoch", leave=False):
        ...
        for i in tqdm(training_index, desc="train_ind", leave=False):
            ...
            dev=G.decoder.decode[0].weight.device
            mean, logvar, rec_adj = G(adj.to(dev))

            noisev = torch.randn(mean.shape, requires_grad=True).to(mean.device)
            noisev = cat_attr(noisev, attr_vec)
            rec_noise = G.decoder(noisev)

            e = int(np.sum(train_adj_mats[i])) // 2

            c_adj = topk_adj(F.sigmoid(rec_adj), e * 2).to(dev)
            c_noise = topk_adj(F.sigmoid(rec_noise), e * 2).to(dev)
        ...
        print("Training set")
        for i in range(min(3,len(train_adj_mats))):
...
def community100():
    ...
    hpars = attr.asdict(
        AttrOptions(
            output_dir="community100", DATA_DIR="data/community100/", av_size=0, d_size=50
        )
    )
...
def run(hpars, _run, _config, our_dataset, batch_size):
    ...
    G.eval()
    Gw = G_Wrap(G.to("cpu"), train_adj_mats, opt.z_size, opt.d_size, attr_vecs=train_attr_vecs)
